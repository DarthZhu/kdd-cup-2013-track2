...
####################################################################################
# Do something with the null names
authors['Name'][pd.isnull(authors['Name'])] = 'missing'
# The next 4 lines are used later for cases that are like "GW Bush" (First initial, Middle Initial, Last name)
authors['GW Bush'] = 0
...
#len(authors.index[authors['Name'].str.contains(" ii$")])
#authors['Name'][authors.index[authors['Name'].str.contains(" ii$")]]
# Foreign Language Special Characters
authors['Name'] = authors['Name'].str.replace("^Jr\. ", "") # 46
...
authors['Name'] = authors['Name'].str.replace("phillippe", "phillipe")# also gregs
...
for row in authors.index:
    ...
    smushed2 = " ".join(authors['Name'][row].split()[:-2]) + " " + authors['Name'][row].split()[-2] + authors['Last'][
        row]
    if smushed in names:
        ansm1 += 1
...
print sm0, ch1, ch2
###################################################################################
#authors['Name'] = authors['Name'].str.replace(" de ", " d ")
###################### Asian-Name Names ############################################
authors['Last'] = authors['Name'].str.split().str[-1]
authors['First'] = authors['Name'].str.split().str[0]    # First name only
ansm3, ansm4 = 0, 0
...
authors['First'] = authors['Name'].str.split().str[0]    # First name only
b1, b2 = 0, 0
authors['Rev'] = authors['Last'] + " " + authors['First']
author_set = authors.groupby(['Name']).groups
...
for row in authors.index:
    ...
    if author_set.__contains__(authors['Rev'][row]):
        num_name = float(dict_len(author_set, authors['Name'][
            row]))      # Higher is bad, ratio may matter more. Ratio may mean reverse
        new_name = float(
            dict_len(author_set, authors['Rev'][row])) # Higher is good, ratio may matter more. Ratio may mean reverse
        num_last = float(
            dict_len(author_last_set, authors['Last'][row]))      # Higher is definately bad, ratio...not sure
        new_last = float(dict_len(author_last_set, authors['First'][row])) # Higer is definately good, ratio...not sure
        num_first = float(
            dict_len(author_first_set, authors['First'][row]))      # Higher is definately bad, ratio...not sure
        new_first = float(dict_len(author_first_set, authors['Last'][row])) # Higer is definately good, ratio...not sure
        ...
        if test_ratio <= 1:
            test = num_name * num_name * np.sqrt(name_ratio) * num_last * np.sqrt(last_ratio) * num_first * np.sqrt(
                first_ratio)
            ...
            if row in affs.index:
                for row2 in authors.index[authors['Name'] == authors['NewN'][row]]:
                    if row2 in affs.index and util.check_affs(affs['Affiliation'][row], affs['Affiliation'][row2]):
                        print "XXXXXXXXXX"
                        test /= 100
            print int(test), "\t", int(num_name), int(new_name), "|", int(num_first), int(new_first), "|", int(
                num_last), "|", int(num_fl), "\t\t",
            if test > 202:
                r2 += 1
...
for row in authors.index:
    if author_set.__contains__(authors['Name'][row][:-1]):
        ...
        if test_ratio <= 1: # sqrt of firsts is better???
            ...
            if row in affs.index:
                for row2 in authors.index[authors['Name'] == authors['Name'][row][:-1]]:
                    if row2 in affs.index and util.check_affs(affs['Affiliation'][row], affs['Affiliation'][row2]):
                        print "XXXXXXXXXX"
                        test /= 100
            print num_long_name, num_shrt_name, num_long_last, num_shrt_last, num_firsts, "\t", int(test), "\t", \
                authors['Name'][row],
            ...
            test = num_shrt_name * (1 / np.sqrt(name_ratio)) * np.sqrt(num_firsts) * num_shrt_last * (
...
for row in authors.index:
    if authors['Name'][row][-1] == 'i' and authors['Name'][row][:-1] + "y" in author_set:
        num_old_name = float(dict_len(author_set, authors['Name'][
            row]))      # Higher is bad, ratio may matter more. Ratio may mean reverse
        num_new_name = float(dict_len(author_set, authors['Name'][row][:-1] + "y")) # Higher is good, ratio may matter more. Ratio may mean reverse
        num_old_last = float(
            dict_len(author_last_set, authors['Last'][row]))      # Higher is definately bad, ratio...not sure
        num_new_last = float(
            dict_len(author_last_set, authors['Last'][row][:-1] + "y")) # Higer is definately good, ratio...not sure
...
for row in authors.index:
    if authors['Name'][row][0] == 'k' and ("c" + authors['Name'][row][1:]) in author_set:
        ...
        num_old_first = float(
            dict_len(author_first_set, authors['Firsts'][row]))      # Higher is definately bad, ratio...not sure
        num_new_first = float(
            dict_len(author_first_set, "c" + authors['Firsts'][row][1:])) # Higer is definately good, ratio...not sure
        ...
        if test_ratio <= 4:
            ...
            test = num_new_name * (1 / np.sqrt(name_ratio)) * np.sqrt(num_last) * num_new_first * (
                1 / np.sqrt(first_ratio))
            print num_old_name, num_new_name, num_old_first, num_new_first, num_last, "\t", int(test), "\t", \
...
for row in authors.index:
    if authors['Name'][row][-3:] == 'and' and (authors['Name'][row][:-3]) in author_set:
        ...
        num_old_last = float(
            dict_len(author_last_set, authors['Last'][row]))      # Higher is definately bad, ratio...not sure
        num_new_last = float(
            dict_len(author_last_set, authors['Last'][row][:-3])) # Higer is definately good, ratio...not sure
        ...
        if test_ratio <= 4:
            ...
            test = num_new_name * (1 / np.sqrt(name_ratio)) * np.sqrt(num_first) * num_new_last * (
                1 / np.sqrt(last_ratio))
            print num_old_name, num_new_name, num_old_last, num_new_last, num_first, "\t", int(test), "\t", \
...
for row in authors.index:
    if authors['Name'][row][-2:] == 'yz' and (authors['Name'][row][:-2]) in author_set:
        ...
        num_old_last = float(
            dict_len(author_last_set, authors['Last'][row]))      # Higher is definately bad, ratio...not sure
        num_new_last = float(
            dict_len(author_last_set, authors['Last'][row][:-2])) # Higer is definately good, ratio...not sure
        num_first = dict_len(author_first_set, authors['Firsts'][row])       # Lower is definately good
        name_ratio = (num_old_name / num_new_name)
        last_ratio = (num_old_last / num_new_last)
        test_ratio = name_ratio * last_ratio
                            ...
                            if test < 5:
                                if test < 0.071: #09
                                    ...
                                    if test > 0.01:
                                        ...
                                        print test, "\t", num_last, "|", int(num_n1), int(
                                            num_n2), "|", shorter, "\tsame last"
                                else:
                                    #print "\t\t\t", aid, lev, name, "|", coa
                    ...
                    if test_ratio < 1: # changing to the 1st one, the ag one
                        ...
                        if test < 5:
                            if test < 0.30: # 09
                                ...
                                if test > 0.01:
                                    print aid, lev, name, "|", coa
                                    print test, "\t", num_firsts, "|", int(num_n1), int(
                                        num_n2), "|", shorter, "\tsame firsts"
                            else:
                                #print "\t\t\t", aid, lev, name, "|", coa
...
####################################################################################
#### SLOW runtime about 55 mins 
# Names within names (look for rares last names) # also long names (4 within 5)
authors['Last'] = authors['Name'].str.split().str[-1]
authors['Firsts'] = authors['Name'].str.split().str[:-1]
authors['Firsts'] = authors['Firsts'].str.join(' ')
count3FL = 0
...
author_last_set = authors.groupby(['Last']).groups
author_first_set = authors.groupby(['First']).groups
author_firsts_set = authors.groupby(['Firsts']).groups
for row in sorted(authors.index):
    if len(authors['Name'][row].split()) == 3:
        # Cut First Name
        if " ".join(authors['Name'][row].split()[1:]) in author_set:
            long_name = authors['Name'][row]
            shrt_name = " ".join(authors['Name'][row].split()[1:])
            mid = authors['Name'][row].split()[1]
            count_long = float(dict_len(author_set, long_name))
            count_shrt = float(dict_len(author_set, shrt_name))
            count_first = float(dict_len(author_first_set, authors['First'][row]))
            count_firsts = float(dict_len(author_firsts_set, authors['Firsts'][row]))
            count_mid = float(dict_len(author_first_set, mid))
            count_last = float(dict_len(author_last_set, authors['Last'][row]))
            ...
            if test_ratio >= 1: # make it the longer name
                test = count_last * count_shrt * count_shrt * count_shrt * count_mid * count_mid * count_mid / (
                    count_first * count_firsts * count_long)
                if row in affs.index and long_name in author_set:
                    for row2 in author_set[long_name]:
                        if row2 in affs.index and util.check_affs(affs['Affiliation'][row], affs['Affiliation'][row2]):
                            print row, row2, "XXXXXXXXXXXXXXXX"
                            test /= 100
                if test < 25:
                    count3FL += 1
                    cur_idx = author_set[shrt_name]
                    dict_move(author_set, shrt_name, long_name)
                    authors['Name'][cur_idx] = long_name
                    print "A", row, int(
                        test), "|", count_last, "|", count_long, count_shrt, "|", count_first, count_firsts, count_mid, shrt_name, "\t\t", long_name
            else:
                test = count_last * count_long * count_long * count_long * count_first * count_mid * count_firsts / (
                    count_shrt * count_shrt)
                if row in affs.index and shrt_name in author_set:
                    for row2 in author_set[shrt_name]:
                        if row2 in affs.index and util.check_affs(affs['Affiliation'][row], affs['Affiliation'][row2]):
                            print row, row2, "XXXXXXXXXXXXXXXX"
                            test /= 100
                if test < 250:
                    count3FS += 1
                    dict_move(author_set, authors['Name'][row], shrt_name, row)
                    authors['Name'][row] = shrt_name
                    print "B", row, int(
                        test), "|", count_last, "|", count_long, count_shrt, "|", count_first, count_firsts, count_mid, long_name, "\t\t", shrt_name
        # cut last name
        elif " ".join(authors['Name'][row].split()[:2]) in author_set:
            long_name = authors['Name'][row]
            ...
            count_firsts = float(dict_len(author_firsts_set, authors['Firsts'][row]))
            count_last = float(dict_len(author_last_set, authors['Last'][row]))
            count_mid = float(dict_len(author_last_set, mid))
            name_ratio = (count_long / count_shrt)
            ...
            test = count_first * count_long * count_long * count_long * count_last * count_last * count_last * count_firsts / (
                count_shrt * count_shrt)
            if row in affs.index and shrt_name in author_set:
                for row2 in author_set[shrt_name]:
                    if row2 in affs.index and util.check_affs(affs['Affiliation'][row], affs['Affiliation'][row2]):
            ...
            if test < 100:
                ...
                authors['Name'][row] = shrt_name
                print "C", row, int(
                    test), "|", count_firsts, count_first, "|", count_long, count_shrt, "|", count_last, count_mid, long_name, "\t\t", shrt_name
        ...
        if " ".join(authors['Name'][row].split()[1:]) in author_set:
            ...
            count_long = float(dict_len(author_set, long_name))
            count_shrt = float(dict_len(author_set, shrt_name))
            count_cut = float(dict_len(author_firsts_set, authors['Firsts'][row]))
            count_new = float(dict_len(author_firsts_set, new_firsts))
            name_ratio = (count_long / count_shrt)
            ...
            if test_ratio >= 1: # make it the longer name
                ...
                authors['Name'][row] = shrt_name
        # cut last name
        elif " ".join(authors['Name'][row].split()[:3]) in author_set:
            long_name = authors['Name'][row]
            shrt_name = " ".join(authors['Name'][row].split()[:3])
            new_last = " ".join(authors['Name'][row].split()[2:3])
            count_long = float(dict_len(author_set, long_name))
            count_shrt = float(dict_len(author_set, shrt_name))
            count_cut = float(dict_len(author_last_set, authors['Last'][row]))
            count_new = float(dict_len(author_last_set, new_last))
            name_ratio = (count_long / count_shrt)
    ...
    # cut last names
        ...
        if " ".join(authors['Name'][row].split()[:4]) in author_set:
            long_name = authors['Name'][row]
            ...
            if test_ratio >= 1: # make it the longer name
                ...
                dict_move(author_set, authors['Name'][row], shrt_name, row)
                authors['Name'][row] = shrt_name
        elif " ".join(authors['Name'][row].split()[1:]) in author_set:
            long_name = authors['Name'][row]
            shrt_name = " ".join(authors['Name'][row].split()[1:])
            new_firsts = " ".join(authors['Name'][row].split()[1:-1])
            count_long = float(dict_len(author_set, long_name))
...
####################################################################################
####################################################################################
# Short Names like Ben, Chris, etc..
